...
Usage: ${SCRIPT_NAME} [-Nth] [-1 CPU] [-p PREFIX] [-P CONTROLPORT] [-T TX_SECONDS] -c CONF
...
while getopts "Nt1:p:P:T:c:h" opt; do
  case "$opt" in
    N) USE_NETNS=true ;;
    t) USE_TS2PHC=true ;;
    1) CPU_ATS1=$OPTARG ;;
...
while true; do
  ...
  echo "CIR $cir_input" | nc_send "$RX_IP" "$CONTROLPORT" "$TX_NETNS_NAME"
