...
write_csv_header_if_needed "$CSVFILE" \
  "run_datetime,cir_mbps,interval_mean_ns,interval_median_ns,interval_max_ns,interval_min_ns,interval_max_minus_min_ns,interval_sd_ns,cpu_ats,cpu_iperf,tx_duration_s,use_ts2phc,use_netns,isolcpus_list,script_version,tx_command,rx_command,pcap_file"
...
while true; do
  ...
  if [[ "$cir_input_str" == "PRIOCIR "* ]]; then
    cir_input=${cir_input_str#PRIOCIR }
  ...
  iperf_log_file="${PREFIX}cir${cir_input}Mbps.dur${TX_SECONDS_META}.${count}.iperf3.log"
  ...
  echo ">>> starting iperf3 server: ${iperf_log_file}"

  # iperf3 server
  if [[ $USE_NETNS == true ]]; then
    ip netns exec "$RX_NETNS_NAME" iperf3 -s >> "${iperf_log_file}" 2>&1 &
    IPERF_SERV_PID=$!
  else 
    iperf3 -s >> "${iperf_log_file}" 2>&1 &
    IPERF_SERV_PID=$!
  fi
  ...
  echo "- iperf3 server log: ${iperf_log_file}" | tee -a "$STATSFILE"
  ...
  tee -a "$STATSFILE" < "${iperf_log_file}"
  echo "==============================================" | tee -a "$STATSFILE"
